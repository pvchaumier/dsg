...
from helpers import horizontal_flip, vertical_flip, transposition
...
# Evaluation function
def predict(f_eval, x, i, batch_size):
    """Better evaluation by using transformation of the images.

    x is a batch.

    The class is obtained by modifying the input image with different 
    transformation and evaluating each of the modified image. The final class
    is obtained by a weighted sum.
    """

    x_rot180 = [rotate(img, 180, resize=True, mode='wrap') 
                    for img in x[i:i + batch_size]]
    x_horiz_flip = [horizontal_flip(img, 180, resize=True, mode='wrap') 
                        for img in x[i:i + batch_size]]
    x_vert_flip = [vertical_flip(img, 180, resize=True, mode='wrap') 
                       for img in x[i:i + batch_size]]

    # to add for better precision but be careful as the prediction should be
    # switched from North-South orientation to East-West orientation and vice
    # versa
    ...
    # not sure about this sum ...
    return (f_eval(x[i:i + batch_size]) + 
            f_eval(x_rot180) + 
            f_eval(x_horiz_flip) +
            f_eval(x_vert_flip))
...
def evaluate(x, y, batch_size=100):
    ...
    for i in xrange(0, len(x), batch_size):
        ...
        count_correct += np.sum(predict(f_eval, x, i, batch_size).argmax(axis=1) == y[i:i + batch_size])
...
def write_predictions(idx, x, batch_size=100):
    ...
    for j in xrange(0, len(x), batch_size):
        predictions.append(list(predict(f_eval, x, j, batch_size).argmax(axis=1)))
